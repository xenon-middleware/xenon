...
public class FtpFileAdaptor extends FileAdaptor {
    ...
    public FileSystem newFileSystem(String location, Credential credential, Map<String, String> properties)
        ...
        return createAndRegisterFileSystem(getName(), location, credential, xenonProperties, ftpClient);
    }
    ...
    private FileSystemImplementation createAndRegisterFileSystem(String scheme, String location, Credential credential,
        ...
        FileSystemImplementation fileSystem = new FileSystemImplementation(getName(), uniqueID, scheme, location,
    ...
    public void close(FileSystem fileSystem) throws XenonException {
        ...
        if (!isOpen(fileSystem)) {
            throw new XenonException(getName(), "File system is already closed");
        }
        ...
        try {
            ...
            throw new XenonException(getName(), "Exception while disconnecting ftp file system.", e);
        }
    ...
    private void assertSameFileSystemsForMove(Path source, Path target) throws XenonException {
        ...
        if (!sourcefs.getLocation().equals(targetfs.getLocation())) {
            throw new XenonException(getName(), "Cannot move between different FileSystems: " + sourcefs.getLocation()
    ...
    private List<FTPFile> listDirectory(Path path, Filter filter) throws XenonException {
        ...
        try {
            if (filter == null) {
                throw new XenonException(getName(), "Filter is null.");
            }
    ...
    public DirectoryStream<PathAttributesPair> newAttributesDirectoryStream(Path path, Filter filter) throws XenonException {
        ...
        if (path == null) {
            throw new XenonException(getName(), "Cannot open attribute directory stream of null path");
        }
    ...
    public OutputStream newOutputStream(Path path, OpenOption... options) throws XenonException {
        ...
        OpenOptions processedOptions = OpenOptions.processOptions(getName(), options);
    ...
    private Path getPathOnNewFileSystem(Path path) throws XenonException {
        ...
        FileSystem newFileSystem = newFileSystem(fileSystem.getLocation(), credential, fileSystem.getProperties());
    ...
    public Path readSymbolicLink(Path path) throws XenonException {
        ...
        throw new XenonException(getName(), "Ftp file system does not support symbolic links");
    }
    ...
    private void assertPathNotExists(Path path) throws XenonException {
        if (exists(path)) {
            throw new PathAlreadyExistsException(getName(), "File already exists: " + path);
        }
    }

    private void assertPathExists(Path path) throws XenonException {
        if (!exists(path)) {
            throw new NoSuchPathException(getName(), "File does not exist: " + path);
        }
    }
