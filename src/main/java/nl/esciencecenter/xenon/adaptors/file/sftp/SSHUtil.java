...
public class SSHUtil {
	...
	/*
	SshClient createSSHClient(XenonProperties properties) { 

		SshClient client = SshClient.setUpDefaultClient();
		
		if (properties.getBooleanProperty(SftpProperties.LOAD_STANDARD_KNOWN_HOSTS)) {
			client.setServerKeyVerifier(new DefaultKnownHostsServerKeyVerifier(RejectAllServerKeyVerifier.INSTANCE));
		}

		if (properties.getBooleanProperty(SftpProperties.LOAD_SSH_CONFIG)) {
			client.setHostConfigEntryResolver(DefaultConfigFileHostEntryResolver.INSTANCE);
		}
		
		if (properties.getBooleanProperty(SftpProperties.AGENT)) {
			client.setAgentFactory(new ProxyAgentFactory());
		}

		if (properties.getBooleanProperty(SftpProperties.AGENT_FORWARDING)) {
...
//		return createSSHClient(true, true, false, false);
//	}
//	
//	public SshClient createSSHClient(boolean loadKnownHosts, boolean loadSSHConfig) { 
//		return createSSHClient(loadKnownHosts, loadSSHConfig, false, false);
//	}
